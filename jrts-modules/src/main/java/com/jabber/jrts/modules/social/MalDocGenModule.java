...
@JRTSModule(id="gen-maldocgen",name="Malicious Doc Builder",description="Generates macro-enabled document payloads.",category=Category.SOCIAL_ENGINEERING,riskLevel=RiskLevel.CRITICAL)
public class MalDocGenModule implements JRTSModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-maldocgen");
            try {
                ctx.log("[*] Malicious Doc Builder initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing Malicious Doc Builder...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","Malicious Doc Builder");out.put("category","SOCIAL_ENGINEERING");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] Malicious Doc Builder finished successfully");result.addFinding(out);result.complete(out);
