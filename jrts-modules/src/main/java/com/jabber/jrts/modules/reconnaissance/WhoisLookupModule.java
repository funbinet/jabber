...
@JRTSModule(id="gen-whoislookup",name="WHOIS Lookup",description="Domain/IP registration lookup via WHOIS protocol for intelligence gathering.",category=Category.RECONNAISSANCE,riskLevel=RiskLevel.LOW)
public class WhoisLookupModule implements JRTSModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-whoislookup");
            try {
                String target=g(input,"target");ctx.log("[*] WHOIS: "+target);ctx.reportProgress(10);
                java.net.Socket s=new java.net.Socket("whois.iana.org",43);s.setSoTimeout(8000);s.getOutputStream().write((target+"\r\n").getBytes());
                java.io.BufferedReader r=new java.io.BufferedReader(new java.io.InputStreamReader(s.getInputStream()));
                String line;StringBuilder sb=new StringBuilder();while((line=r.readLine())!=null){sb.append(line).append("\n");if(!line.startsWith("%"))ctx.log("[+] "+line);}
                s.close();Map<String,Object>out=new LinkedHashMap<>();out.put("target",target);out.put("raw",sb.toString());result.addFinding(out);result.complete(out);
