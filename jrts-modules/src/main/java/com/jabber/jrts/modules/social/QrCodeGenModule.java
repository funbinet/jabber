...
@JRTSModule(id="gen-qrcodegen",name="QR Code Generator",description="Generates QR codes with malicious data.",category=Category.SOCIAL_ENGINEERING,riskLevel=RiskLevel.MEDIUM)
public class QrCodeGenModule implements JRTSModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-qrcodegen");
            try {
                ctx.log("[*] QR Code Generator initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing QR Code Generator...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","QR Code Generator");out.put("category","SOCIAL_ENGINEERING");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] QR Code Generator finished successfully");result.addFinding(out);result.complete(out);
