...
@JRTSModule(id="gen-emailspoofer",name="Email Spoofer",description="Crafts spoofed SMTP email headers.",category=Category.SOCIAL_ENGINEERING,riskLevel=RiskLevel.CRITICAL)
public class EmailSpooferModule implements JRTSModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-emailspoofer");
            try {
                ctx.log("[*] Email Spoofer initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing Email Spoofer...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","Email Spoofer");out.put("category","SOCIAL_ENGINEERING");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] Email Spoofer finished successfully");result.addFinding(out);result.complete(out);
