...
export async function fetchModuleTools(id) {
  const res = await fetch(`${API_BASE}/modules/${id}/tools`);
  return res.json();
}

export async function fetchModuleToolsForMode(id, mode) {
  const res = await fetch(`${API_BASE}/modules/${id}/tools?mode=${encodeURIComponent(mode)}`);
  return res.json();
}

export async function downloadModuleTool(moduleId, toolId) {
  const res = await fetch(`${API_BASE}/modules/${moduleId}/tools/${toolId}/download`, {
    method: 'POST',
  });
  return res.json();
}

export async function fetchModuleToolDownloadStatus(moduleId, toolId) {
  const res = await fetch(`${API_BASE}/modules/${moduleId}/tools/${toolId}/download`);
  return res.json();
}

export async function deleteModuleTool(moduleId, toolId) {
  const res = await fetch(`${API_BASE}/modules/${moduleId}/tools/${toolId}`, {
    method: 'DELETE',
  });
  return res.json();
}

export async function executeModule(moduleId, input, selectedTools = [], sudoPassword = '') {
  const selectedToolsString = Array.isArray(selectedTools) ? selectedTools.join(',') : selectedTools;
  const payload = { ...input, selectedTools: selectedToolsString };
...
// =============== V 5.5: TARGET PROFILING ===============

export async function generateProfile(reportIds, save = false, format = 'html') {
...
// =============== V 5.5: UNIVERSAL FILE SERVING ===============

export function getServeUrl(reportId) {
  return `${API_BASE}/reports/${reportId}/serve`;
}
