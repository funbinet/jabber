package com.jabber.jabber.core.api;

import com.jabber.jabber.data.model.*;
import com.jabber.jabber.core.engine.TaskEngine;
...
import com.jabber.jabber.core.profiling.TargetProfileEngine;
...
public class JABBERApiController {
    ...
    public JABBERApiController(PluginRegistry registry, TaskEngine taskEngine,
    ...
    public Map<String, Object> getSystemInfo() {
        ...
        info.put("name", "JABBER");
        info.put("version", "V 5.5.0");
    ...
    public ResponseEntity<List<Map<String, Object>>> getModuleSchema(@PathVariable String id) {
        JABBERModuleInterface module = registry.getModule(id);
        if (module == null)
            return ResponseEntity.notFound().build();
    ...
    // ===== Module Tooling =====
    @GetMapping("/modules/{id}/tools")
    public ResponseEntity<List<Map<String, Object>>> getModuleTools(@PathVariable String id,
            @RequestParam(required = false) String mode) {
        JABBERModuleInterface module = registry.getModule(id);
        if (module == null)
            return ResponseEntity.notFound().build();
        if (module instanceof ToolingModule toolingModule) {
            if (mode != null && !mode.isEmpty()) {
                return ResponseEntity.ok(toolingModule.getToolStatusesForMode(mode));
            }
            return ResponseEntity.ok(toolingModule.getToolStatuses());
        }
        return ResponseEntity.ok(List.of());
    }

    @PostMapping("/modules/{id}/tools/{toolId}/download")
    public ResponseEntity<Map<String, Object>> downloadModuleTool(@PathVariable String id,
            @PathVariable String toolId) {
        JABBERModuleInterface module = registry.getModule(id);
        if (module == null)
            return ResponseEntity.notFound().build();
        if (module instanceof ToolingModule toolingModule) {
            return ResponseEntity.ok(toolingModule.downloadTool(toolId));
        }
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(Map.of("error", "Module does not expose tooling operations"));
    }

    @GetMapping("/modules/{id}/tools/{toolId}/download")
    public ResponseEntity<Map<String, Object>> getModuleToolDownloadStatus(@PathVariable String id,
            @PathVariable String toolId) {
        JABBERModuleInterface module = registry.getModule(id);
        if (module == null)
            return ResponseEntity.notFound().build();
        if (module instanceof ToolingModule toolingModule) {
            return ResponseEntity.ok(toolingModule.getToolDownloadStatus(toolId));
        }
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(Map.of("error", "Module does not expose tooling operations"));
    }

    @DeleteMapping("/modules/{id}/tools/{toolId}")
    public ResponseEntity<Map<String, Object>> deleteModuleTool(@PathVariable String id, @PathVariable String toolId) {
        JABBERModuleInterface module = registry.getModule(id);
        if (module == null)
            return ResponseEntity.notFound().build();
        if (module instanceof ToolingModule toolingModule) {
            return ResponseEntity.ok(toolingModule.deleteTool(toolId));
        }
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(Map.of("error", "Module does not expose tooling operations"));
    }
    ...
    /**
     * Get report content by ID (text-based).
     */
    ...
    /**
     * V5.5: Universal file serving with proper MIME type for inline rendering.
     * Supports images, videos, PDFs, scripts, captures — everything in the artifacts gallery.
     */
    @GetMapping("/reports/{id}/serve")
    public ResponseEntity<byte[]> serveReportFile(@PathVariable String id) {
        try {
            byte[] data = storage.getReportBytes(id);
            if (data == null)
                return ResponseEntity.notFound().build();

            Map<String, Object> meta = storage.getReportMeta(id);
            String mimeType = meta != null ? (String) meta.get("mimeType") : null;
            if (mimeType == null || mimeType.isBlank()) {
                String filePath = meta != null ? (String) meta.get("filePath") : null;
                String format = meta != null ? (String) meta.get("format") : null;
                mimeType = com.jabber.jabber.data.model.ReportMetadata.guessMimeType(filePath, format);
            }

            String fileName = "";
            if (meta != null && meta.get("filePath") != null) {
                String fp = (String) meta.get("filePath");
    ...
    public ResponseEntity<Resource> downloadArtifact(@RequestParam String path) {
        try {
            // Safety measure: sanitize path to prevent directory traversal outside of
            // reports
            if (path.contains(".."))
    ...
    public ResponseEntity<Map<String, Object>> editReport(@PathVariable String id,
    ...
    public ResponseEntity<Map<String, Object>> renameReport(@PathVariable String id,
            @RequestBody Map<String, String> body) {
        try {
