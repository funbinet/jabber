...
@JABBERModule(id="gen-pathintercept",name="PATH Interception Finder",description="Finds PATH interception and DLL hijack vectors.",category=Category.PRIVILEGE_ESCALATION,riskLevel=RiskLevel.MEDIUM)
public class PathInterceptModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-pathintercept");
            try {
                ctx.log("[*] PATH Interception Finder initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing PATH Interception Finder...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","PATH Interception Finder");out.put("category","PRIVILEGE_ESCALATION");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] PATH Interception Finder finished successfully");result.addFinding(out);result.complete(out);
