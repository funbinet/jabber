...
@JABBERModule(id="gen-sleepmaskgen",name="Sleep Mask Generator",description="Creates sleep-masking evasion payloads.",category=Category.PAYLOAD_CREATION,riskLevel=RiskLevel.HIGH)
public class SleepMaskGenModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-sleepmaskgen");
            try {
                ctx.log("[*] Sleep Mask Generator initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing Sleep Mask Generator...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","Sleep Mask Generator");out.put("category","PAYLOAD_CREATION");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] Sleep Mask Generator finished successfully");result.addFinding(out);result.complete(out);
