...
@JABBERModule(id="gen-eventlogexport",name="Event Log Exporter",description="Exports and filters Windows event logs.",category=Category.FORENSICS,riskLevel=RiskLevel.MEDIUM)
public class EventLogExportModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-eventlogexport");
            try {
                ctx.log("[*] Event Log Exporter initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing Event Log Exporter...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","Event Log Exporter");out.put("category","FORENSICS");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] Event Log Exporter finished successfully");result.addFinding(out);result.complete(out);
