...
@JABBERModule(id="gen-zipcrack",name="ZIP Password Cracker",description="Brute-forces ZIP passwords.",category=Category.PASSWORD_CRACKING,riskLevel=RiskLevel.MEDIUM)
public class ZIPCrackModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-zipcrack");
            try {
                ctx.log("[*] ZIP Password Cracker initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing ZIP Password Cracker...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","ZIP Password Cracker");out.put("category","PASSWORD_CRACKING");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] ZIP Password Cracker finished successfully");result.addFinding(out);result.complete(out);
