...
@JABBERModule(id="gen-psexecnative",name="PsExec Native",description="Remote command exec via SMB service creation.",category=Category.LATERAL_MOVEMENT,riskLevel=RiskLevel.CRITICAL)
public class PsExecNativeModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-psexecnative");
            try {
                ctx.log("[*] PsExec Native initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing PsExec Native...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","PsExec Native");out.put("category","LATERAL_MOVEMENT");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] PsExec Native finished successfully");result.addFinding(out);result.complete(out);
