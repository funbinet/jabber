...
@JABBERModule(id="gen-pdfwrap",name="PDF Report Wrapper",description="Converts reports to PDF format.",category=Category.REPORTS,riskLevel=RiskLevel.LOW)
public class PDFWrapModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-pdfwrap");
            try {
                ctx.log("[*] PDF Report Wrapper initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing PDF Report Wrapper...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","PDF Report Wrapper");out.put("category","REPORTS");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] PDF Report Wrapper finished successfully");result.addFinding(out);result.complete(out);
