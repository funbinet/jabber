...
@JABBERModule(id="gen-regparser",name="Registry Hive Parser",description="Parses Windows registry hives.",category=Category.FORENSICS,riskLevel=RiskLevel.MEDIUM)
public class RegParserModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-regparser");
            try {
                ctx.log("[*] Registry Hive Parser initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing Registry Hive Parser...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","Registry Hive Parser");out.put("category","FORENSICS");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] Registry Hive Parser finished successfully");result.addFinding(out);result.complete(out);
