...
@JABBERModule(id="gen-ntdsditdump",name="NTDS.dit Extractor",description="Extracts hashes from AD NTDS.dit.",category=Category.CREDENTIAL_ACCESS,riskLevel=RiskLevel.CRITICAL)
public class NTDSDitDumpModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-ntdsditdump");
            try {
                ctx.log("[*] NTDS.dit Extractor initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing NTDS.dit Extractor...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","NTDS.dit Extractor");out.put("category","CREDENTIAL_ACCESS");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] NTDS.dit Extractor finished successfully");result.addFinding(out);result.complete(out);
