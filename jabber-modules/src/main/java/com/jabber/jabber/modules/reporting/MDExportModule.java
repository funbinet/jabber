...
@JABBERModule(id="gen-mdexport",name="Markdown Exporter",description="Creates markdown reports for documentation.",category=Category.REPORTS,riskLevel=RiskLevel.LOW)
public class MDExportModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-mdexport");
            try {
                ctx.log("[*] Markdown Exporter initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing Markdown Exporter...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","Markdown Exporter");out.put("category","REPORTS");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] Markdown Exporter finished successfully");result.addFinding(out);result.complete(out);
