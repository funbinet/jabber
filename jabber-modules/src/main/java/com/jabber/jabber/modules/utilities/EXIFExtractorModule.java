...
@JABBERModule(id="gen-exifextractor",name="EXIF Metadata Extractor",description="Extracts EXIF metadata from images.",category=Category.FORENSICS,riskLevel=RiskLevel.LOW)
public class EXIFExtractorModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-exifextractor");
            try {
                ctx.log("[*] EXIF Metadata Extractor initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing EXIF Metadata Extractor...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","EXIF Metadata Extractor");out.put("category","FORENSICS");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] EXIF Metadata Extractor finished successfully");result.addFinding(out);result.complete(out);
