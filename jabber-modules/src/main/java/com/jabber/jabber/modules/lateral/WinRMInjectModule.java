...
@JABBERModule(id="gen-winrminject",name="WinRM Shell",description="Remote PowerShell via WinRM.",category=Category.LATERAL_MOVEMENT,riskLevel=RiskLevel.HIGH)
public class WinRMInjectModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-winrminject");
            try {
                ctx.log("[*] WinRM Shell initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing WinRM Shell...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","WinRM Shell");out.put("category","LATERAL_MOVEMENT");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] WinRM Shell finished successfully");result.addFinding(out);result.complete(out);
