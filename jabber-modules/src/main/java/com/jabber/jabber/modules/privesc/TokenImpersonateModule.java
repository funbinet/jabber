...
@JABBERModule(id="gen-tokenimpersonate",name="Token Impersonation Checker",description="Checks for token impersonation privileges.",category=Category.PRIVILEGE_ESCALATION,riskLevel=RiskLevel.HIGH)
public class TokenImpersonateModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-tokenimpersonate");
            try {
                ctx.log("[*] Token Impersonation Checker initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing Token Impersonation Checker...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","Token Impersonation Checker");out.put("category","PRIVILEGE_ESCALATION");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] Token Impersonation Checker finished successfully");result.addFinding(out);result.complete(out);
