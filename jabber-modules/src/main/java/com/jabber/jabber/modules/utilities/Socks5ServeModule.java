...
@JABBERModule(id="gen-socks5serve",name="SOCKS5 Server",description="Deploys SOCKS5 proxy for routing.",category=Category.C2_PERSISTENCE,riskLevel=RiskLevel.MEDIUM)
public class Socks5ServeModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-socks5serve");
            try {
                ctx.log("[*] SOCKS5 Server initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing SOCKS5 Server...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","SOCKS5 Server");out.put("category","C2_PERSISTENCE");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] SOCKS5 Server finished successfully");result.addFinding(out);result.complete(out);
