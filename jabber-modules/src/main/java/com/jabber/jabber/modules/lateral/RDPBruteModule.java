...
@JABBERModule(id="gen-rdpbrute",name="RDP Credential Tester",description="Tests RDP with credential lists.",category=Category.LATERAL_MOVEMENT,riskLevel=RiskLevel.HIGH)
public class RDPBruteModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-rdpbrute");
            try {
                ctx.log("[*] RDP Credential Tester initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing RDP Credential Tester...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","RDP Credential Tester");out.put("category","LATERAL_MOVEMENT");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] RDP Credential Tester finished successfully");result.addFinding(out);result.complete(out);
