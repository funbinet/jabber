...
@JABBERModule(id="gen-credimport",name="Credential Importer",description="Imports credentials from various formats.",category=Category.SAVED_CREDENTIALS,riskLevel=RiskLevel.MEDIUM)
public class CredImportModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-credimport");
            try {
                ctx.log("[*] Credential Importer initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing Credential Importer...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","Credential Importer");out.put("category","SAVED_CREDENTIALS");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] Credential Importer finished successfully");result.addFinding(out);result.complete(out);
