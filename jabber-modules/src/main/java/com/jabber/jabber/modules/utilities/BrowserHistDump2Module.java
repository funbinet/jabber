...
@JABBERModule(id="gen-browserhistdump2",name="Browser Forensics",description="Extracts browser artifacts for forensic analysis.",category=Category.FORENSICS,riskLevel=RiskLevel.MEDIUM)
public class BrowserHistDump2Module implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-browserhistdump2");
            try {
                ctx.log("[*] Browser Forensics initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing Browser Forensics...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","Browser Forensics");out.put("category","FORENSICS");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] Browser Forensics finished successfully");result.addFinding(out);result.complete(out);
