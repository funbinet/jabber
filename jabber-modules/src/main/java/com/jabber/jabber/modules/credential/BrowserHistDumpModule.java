...
@JABBERModule(id="gen-browserhistdump",name="Browser History Extractor",description="Extracts browser artifacts and history.",category=Category.FORENSICS,riskLevel=RiskLevel.MEDIUM)
public class BrowserHistDumpModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-browserhistdump");
            try {
                ctx.log("[*] Browser History Extractor initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing Browser History Extractor...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","Browser History Extractor");out.put("category","CREDENTIAL_ACCESS");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] Browser History Extractor finished successfully");result.addFinding(out);result.complete(out);
