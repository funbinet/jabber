...
@JABBERModule(id="gen-bpduattack",name="STP BPDU Analyzer",description="Tests STP for BPDU guard vulnerabilities.",category=Category.NETWORK_ATTACK_DEFENSE,riskLevel=RiskLevel.MEDIUM)
public class BPDUAttackModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-bpduattack");
            try {
                ctx.log("[*] STP BPDU Analyzer initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing STP BPDU Analyzer...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","STP BPDU Analyzer");out.put("category","NETWORK_ATTACK_DEFENSE");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] STP BPDU Analyzer finished successfully");result.addFinding(out);result.complete(out);
