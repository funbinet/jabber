...
@JABBERModule(id="gen-subnetcalculator2",name="Quick CIDR Calculator",description="CIDR notation calculator for network planning.",category=Category.UTILITIES,riskLevel=RiskLevel.LOW)
public class SubnetCalculator2Module implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-subnetcalculator2");
            try {
                ctx.log("[*] Quick CIDR Calculator initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing Quick CIDR Calculator...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","Quick CIDR Calculator");out.put("category","UTILITIES");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] Quick CIDR Calculator finished successfully");result.addFinding(out);result.complete(out);
