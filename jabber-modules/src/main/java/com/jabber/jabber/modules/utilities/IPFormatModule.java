...
@JABBERModule(id="gen-ipformat",name="IP Converter",description="Converts IP formats: decimal, hex, octal.",category=Category.UTILITIES,riskLevel=RiskLevel.LOW)
public class IPFormatModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-ipformat");
            try {
                ctx.log("[*] IP Converter initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing IP Converter...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","IP Converter");out.put("category","UTILITIES");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] IP Converter finished successfully");result.addFinding(out);result.complete(out);
