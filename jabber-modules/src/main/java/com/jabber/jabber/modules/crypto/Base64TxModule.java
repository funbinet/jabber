...
@JABBERModule(id="gen-base64tx",name="Base64 Encoder/Decoder",description="Base64/Base64URL encode/decode.",category=Category.CRYPTO_OPERATIONS,riskLevel=RiskLevel.LOW)
public class Base64TxModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-base64tx");
            try {
                ctx.log("[*] Base64 Encoder/Decoder initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing Base64 Encoder/Decoder...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","Base64 Encoder/Decoder");out.put("category","CRYPTO_OPERATIONS");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] Base64 Encoder/Decoder finished successfully");result.addFinding(out);result.complete(out);
