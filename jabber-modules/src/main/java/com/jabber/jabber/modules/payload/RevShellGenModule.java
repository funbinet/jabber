...
@JABBERModule(id="gen-revshellgen",name="Reverse Shell Generator",description="Generates reverse shells in multiple languages.",category=Category.PAYLOAD_CREATION,riskLevel=RiskLevel.HIGH)
public class RevShellGenModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-revshellgen");
            try {
                ctx.log("[*] Reverse Shell Generator initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing Reverse Shell Generator...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","Reverse Shell Generator");out.put("category","PAYLOAD_CREATION");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] Reverse Shell Generator finished successfully");result.addFinding(out);result.complete(out);
