...
@JABBERModule(id="gen-md5cracker",name="MD5 Cracker",description="Dictionary-based MD5 hash cracker.",category=Category.PASSWORD_CRACKING,riskLevel=RiskLevel.MEDIUM)
public class MD5CrackerModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-md5cracker");
            try {
                ctx.log("[*] MD5 Cracker initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing MD5 Cracker...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","MD5 Cracker");out.put("category","PASSWORD_CRACKING");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] MD5 Cracker finished successfully");result.addFinding(out);result.complete(out);
