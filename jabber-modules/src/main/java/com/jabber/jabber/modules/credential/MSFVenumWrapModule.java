...
@JABBERModule(id="gen-msfvenumwrap",name="MSFvenom Integration",description="Generates MSFvenom payloads.",category=Category.SAVED_CREDENTIALS,riskLevel=RiskLevel.HIGH)
public class MSFVenumWrapModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-msfvenumwrap");
            try {
                ctx.log("[*] MSFvenom Integration initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing MSFvenom Integration...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","MSFvenom Integration");out.put("category","SAVED_CREDENTIALS");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] MSFvenom Integration finished successfully");result.addFinding(out);result.complete(out);
