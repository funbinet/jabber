...
@JABBERModule(id="gen-sudoedit",name="Sudo Config Analyzer",description="Parses sudoers for escalation paths.",category=Category.PRIVILEGE_ESCALATION,riskLevel=RiskLevel.MEDIUM)
public class SudoEditModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-sudoedit");
            try {
                ctx.log("[*] Sudo Config Analyzer initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing Sudo Config Analyzer...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","Sudo Config Analyzer");out.put("category","PRIVILEGE_ESCALATION");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] Sudo Config Analyzer finished successfully");result.addFinding(out);result.complete(out);
