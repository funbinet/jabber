...
@JABBERModule(id="gen-dhcpstarve",name="DHCP Starvation Tester",description="Tests DHCP pool exhaustion vulnerability.",category=Category.NETWORK_ATTACK_DEFENSE,riskLevel=RiskLevel.HIGH)
public class DHCPStarveModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-dhcpstarve");
            try {
                ctx.log("[*] DHCP Starvation Tester initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing DHCP Starvation Tester...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","DHCP Starvation Tester");out.put("category","NETWORK_ATTACK_DEFENSE");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] DHCP Starvation Tester finished successfully");result.addFinding(out);result.complete(out);
