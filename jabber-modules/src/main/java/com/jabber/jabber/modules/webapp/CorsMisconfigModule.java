...
@JABBERModule(id="gen-corsmisconfig",name="CORS Misconfiguration Detector",description="Detects CORS misconfigurations allowing unauthorized cross-domain access.",category=Category.WEB_ASSESSMENT,riskLevel=RiskLevel.MEDIUM)
public class CorsMisconfigModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-corsmisconfig");
            try {
                String url=g(input,"url");String origin=g(input,"origin","https://evil.com");ctx.log("[*] CORS: "+url);ctx.reportProgress(20);
java.net.HttpURLConnection c=(java.net.HttpURLConnection)new java.net.URL(url).openConnection();c.setRequestProperty("Origin",origin);c.setConnectTimeout(5000);c.connect();
String acao=c.getHeaderField("Access-Control-Allow-Origin");String acac=c.getHeaderField("Access-Control-Allow-Credentials");boolean vuln=(origin.equals(acao)||"*".equals(acao))&&"true".equals(acac);
Map<String,Object>out=new LinkedHashMap<>();out.put("url",url);out.put("acao",acao);out.put("acac",acac);out.put("vulnerable",vuln);ctx.log("[+] ACAO: "+(acao!=null?acao:"unset"));ctx.log(vuln?"[+] CORS VULN!":"[-] OK");result.addFinding(out);result.complete(out);
