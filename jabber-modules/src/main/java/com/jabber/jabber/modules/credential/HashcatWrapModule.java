...
@JABBERModule(id="gen-hashcatwrap",name="Hashcat Wrapper",description="Hashcat integration for hash cracking.",category=Category.PASSWORD_CRACKING,riskLevel=RiskLevel.HIGH)
public class HashcatWrapModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-hashcatwrap");
            try {
                ctx.log("[*] Hashcat Wrapper initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing Hashcat Wrapper...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","Hashcat Wrapper");out.put("category","PASSWORD_CRACKING");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] Hashcat Wrapper finished successfully");result.addFinding(out);result.complete(out);
