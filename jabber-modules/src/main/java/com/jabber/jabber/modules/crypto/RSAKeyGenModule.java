...
@JABBERModule(id="gen-rsakeygen",name="RSA Key Generator",description="Generates RSA key pairs.",category=Category.CRYPTO_OPERATIONS,riskLevel=RiskLevel.LOW)
public class RSAKeyGenModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-rsakeygen");
            try {
                ctx.log("[*] RSA Key Generator initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing RSA Key Generator...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","RSA Key Generator");out.put("category","CRYPTO_OPERATIONS");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] RSA Key Generator finished successfully");result.addFinding(out);result.complete(out);
