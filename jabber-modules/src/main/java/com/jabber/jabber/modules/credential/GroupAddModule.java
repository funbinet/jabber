...
@JABBERModule(id="gen-groupadd",name="Group Membership Manager",description="Adds users to AD security groups.",category=Category.AD_MANAGEMENT,riskLevel=RiskLevel.HIGH)
public class GroupAddModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-groupadd");
            try {
                ctx.log("[*] Group Membership Manager initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing Group Membership Manager...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","Group Membership Manager");out.put("category","AD_MANAGEMENT");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] Group Membership Manager finished successfully");result.addFinding(out);result.complete(out);
