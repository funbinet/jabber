...
@JABBERModule(id="gen-urldecode",name="URL Encoder/Decoder",description="Encodes/decodes URL-encoded strings.",category=Category.UTILITIES,riskLevel=RiskLevel.LOW)
public class URLDecodeModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-urldecode");
            try {
                ctx.log("[*] URL Encoder/Decoder initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing URL Encoder/Decoder...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","URL Encoder/Decoder");out.put("category","UTILITIES");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] URL Encoder/Decoder finished successfully");result.addFinding(out);result.complete(out);
