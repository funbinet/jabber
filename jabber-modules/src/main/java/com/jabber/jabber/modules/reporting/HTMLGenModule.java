...
@JABBERModule(id="gen-htmlgen",name="HTML Report Generator",description="Generates styled HTML assessment reports.",category=Category.REPORTS,riskLevel=RiskLevel.LOW)
public class HTMLGenModule implements JABBERModuleInterface {
    @Override public List<ModuleInputField> getInputSchema() {
        return List.of(
            ModuleInputField.text("project_name", "Project Name").required().placeholder("JABBER Assessment").group("Project").helpText("Assessment project name").build(),
            ...
            ModuleInputField.text("author", "Author").placeholder("JABBER Operator").group("Project").helpText("Report author").build()
        );
    }
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-htmlgen");
            try {
                ctx.log("[*] HTML Report Generator initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing HTML Report Generator...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","HTML Report Generator");out.put("category","REPORTS");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] HTML Report Generator finished successfully");result.addFinding(out);result.complete(out);
