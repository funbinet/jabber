...
@JABBERModule(id="gen-rot13crypt",name="ROT13 Cipher",description="Caesar cipher with configurable rotation.",category=Category.CRYPTO_OPERATIONS,riskLevel=RiskLevel.LOW)
public class ROT13CryptModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-rot13crypt");
            try {
                ctx.log("[*] ROT13 Cipher initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing ROT13 Cipher...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","ROT13 Cipher");out.put("category","CRYPTO_OPERATIONS");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] ROT13 Cipher finished successfully");result.addFinding(out);result.complete(out);
