...
@JABBERModule(id="gen-samdump",name="SAM Dumper",description="Extracts local hashes from SAM hive.",category=Category.CREDENTIAL_ACCESS,riskLevel=RiskLevel.CRITICAL)
public class SamDumpModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-samdump");
            try {
                ctx.log("[*] SAM Dumper initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing SAM Dumper...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","SAM Dumper");out.put("category","CREDENTIAL_ACCESS");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] SAM Dumper finished successfully");result.addFinding(out);result.complete(out);
