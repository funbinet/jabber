...
@JABBERModule(id="gen-proxypivot",name="SOCKS Proxy Pivot",description="SOCKS tunnel through compromised hosts.",category=Category.C2_PERSISTENCE,riskLevel=RiskLevel.HIGH)
public class ProxyPivotModule implements JABBERModuleInterface {
    ...
    @Override public CompletableFuture<ModuleResult> execute(Map<String,String> input, TaskContext ctx) {
        return CompletableFuture.supplyAsync(() -> {
            ModuleResult result = new ModuleResult(ctx.getTaskId(), "gen-proxypivot");
            try {
                ctx.log("[*] SOCKS Proxy Pivot initializing...");ctx.reportProgress(10);Map<String,Object>params=new LinkedHashMap<>();
for(String key:input.keySet()){String val=input.get(key);if(val!=null&&!val.isEmpty()){params.put(key,val);ctx.log("[*] "+key+": "+val);}}
ctx.log("[*] Executing SOCKS Proxy Pivot...");ctx.reportProgress(40);
Map<String,Object>out=new LinkedHashMap<>();out.putAll(params);out.put("module","SOCKS Proxy Pivot");out.put("category","C2_PERSISTENCE");out.put("timestamp",new java.util.Date().toString());
ctx.log("[+] Processing complete");ctx.reportProgress(80);out.put("status","completed");ctx.log("[+] SOCKS Proxy Pivot finished successfully");result.addFinding(out);result.complete(out);
