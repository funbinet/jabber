...
const Workspace = forwardRef(function Workspace({
  ...
  function renderContent() {
    if (view === 'executor' && activeModule) {
      if (activeModule.category === 'PHONE_ENUMERATION') {
        return <DeviceEnumeratorExecutor module={activeModule} isConnected={isConnected} onBack={onBack} />;
      }
      return <ModuleExecutor module={activeModule} isConnected={isConnected} onBack={onBack} />;
    }
  ...
  return (
    <main className="workspace" id="jrts-workspace" ref={ref} onScroll={onScroll}>
      {renderContent()}
    </main>
  );
});
