...
const InteractiveTerminal = () => {
  ...
  useEffect(() => {
    const node = document.querySelector('.app-body') || document.getElementById('jrts-workspace') || document.body;
    setMountNode(node);
  }, []);
  ...
  return createPortal(
    <div
      className={`jrts-terminal-overlay ${isOpen ? 'is-open' : ''} ${isMinimized ? 'is-minimized' : ''}`}
      aria-hidden={!isOpen}
    >
      <section
        className={`jrts-terminal-panel ${isMinimized ? 'is-minimized' : ''}`}
        role="dialog"
        aria-modal="false"
        aria-label="Terminal"
        ref={panelRef}
      ...
      >
        <header className="jrts-terminal-header">
          <div className="jrts-terminal-title-wrap">
            <TerminalSquare className="jrts-terminal-title-icon" size={16} />
            <span className="jrts-terminal-title">Terminal</span>
            <span className={`jrts-terminal-connection ${connectionClass}`}>
              {connectionLabel}
            </span>
          </div>

          <div className="jrts-terminal-meta">
            <span className="jrts-terminal-meta-item" title={sessionMeta.workingDirectory || 'Unknown path'}>
              {sessionMeta.workingDirectory || 'Directory unavailable'}
            </span>
            <span className="jrts-terminal-meta-item">History: {sessionMeta.historyCount}</span>
          </div>

          <div className="jrts-terminal-actions">
            <button type="button" className="jrts-terminal-action-btn" onClick={handleExport} disabled={isSaving} title="Save Session">
              <Save size={14} />
            </button>
            <button
              type="button"
              className="jrts-terminal-action-btn"
            ...
            <button type="button" className="jrts-terminal-action-btn" onClick={handleClear} title="Clear Terminal">
              <Trash2 size={14} />
            </button>
            <button
              type="button"
            ...
            <button type="button" className="jrts-terminal-action-btn danger" onClick={closeTerminal} title="Close Terminal">
              <X size={14} />
            </button>
          </div>
        </header>

        <div className="jrts-terminal-content">
          <div
            className="jrts-terminal-scrollback"
            ref={terminalRef}
            id="jrts-terminal-output"
            onMouseDown={() => xtermRef.current?.focus()}
          />

          <div className="jrts-terminal-footnote">
            <span>Interactive shell ready: type directly in terminal</span>
            <span>Ctrl+C to interrupt</span>
          </div>

          {effectiveFeedback && (
            <div className="jrts-terminal-feedback">
              {effectiveFeedback}
            </div>
          )}
        </div>
