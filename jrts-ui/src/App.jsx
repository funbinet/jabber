...
function AppInner() {
  ...
  // Settings State
  const [theme, setTheme] = useState(() => localStorage.getItem('jrts_theme') || 'dark');
  const [fontScale, setFontScale] = useState(() => localStorage.getItem('jrts_fontScale') || 'default');
  const [animationsEnabled, setAnimationsEnabled] = useState(() => localStorage.getItem('jrts_animations') !== 'false');
  const [terminalAutoScroll, setTerminalAutoScroll] = useState(() => localStorage.getItem('jrts_terminalAutoScroll') !== 'false');
  const [rawOutputDefault, setRawOutputDefault] = useState(() => localStorage.getItem('jrts_rawOutputDefault') === 'true');
  ...
  useEffect(() => { 
    localStorage.setItem('jrts_theme', theme); 
    document.documentElement.setAttribute('data-theme', theme);
  }, [theme]);
  useEffect(() => { localStorage.setItem('jrts_fontScale', fontScale); }, [fontScale]);
  useEffect(() => { localStorage.setItem('jrts_animations', animationsEnabled); }, [animationsEnabled]);
  useEffect(() => { localStorage.setItem('jrts_terminalAutoScroll', terminalAutoScroll); }, [terminalAutoScroll]);
  useEffect(() => { localStorage.setItem('jrts_rawOutputDefault', rawOutputDefault); }, [rawOutputDefault]);
